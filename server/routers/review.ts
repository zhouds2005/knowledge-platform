...
router.post("/knowledge/:id/submit", requireAuth, async (req: AuthRequest, res) => {
  try {
    const { object, reviewerId } = await submitForReview(req.params.id as string);
    if (reviewerId) {
...
router.post("/knowledge/:id/withdraw", requireAuth, async (req: AuthRequest, res) => {
  try {
    const [obj] = await db.select().from(knowledgeObjects).where(eq(knowledgeObjects.id, req.params.id as string)).limit(1);
...
router.post("/knowledge/:id/approve", requireAuth, async (req: AuthRequest, res) => {
  try {
    const obj = await approveReview(req.params.id as string, req.user!.id);
...
router.post("/knowledge/:id/reject", requireAuth, async (req: AuthRequest, res) => {
  try {
    const { comment } = req.body;
    const obj = await rejectReview(req.params.id as string, req.user!.id, comment);
